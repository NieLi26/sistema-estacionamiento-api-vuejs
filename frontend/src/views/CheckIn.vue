...
import Modal from '@/components/checkin/Modal.vue';

const lots = ref('')

// con axios desde el mismo coponenete
...
// const handleCreatLot = () => {
//   axios.post('http://127.0.0.1:8000/api/lots/',
//   dataPost
//   )
...
// onMounted(() => {
// heredando calls personalizadas
  ...
  <div class="">
    <div class="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8">
      ...
      <div v-if="lots" class="flex flex-wrap -m-4 text-center">
        <a href="#" v-for="lot in lots" :key="lot.id" class="p-4 w-full md:w-1/2 lg:w-1/3 xl:w-1/4 hover:scale-110"
          @click="lot.status === 'av' && (open = true), lotNumber=lot.id">
          ...
          <div class="border-2 border-gray-200  px-4 py-6 rounded-lg hover:shadow-xl " :class="colorLot(lot.status)">
            ...
            <h2 class="title-font font-medium text-2xl text-gray-900"> {{ lot.status === 'bu' ? lot.reserves.at(-1): lot.status === 're' &&  lot.periods.slice(-1)[0]|| 'Estacionamiento' }} </h2>
          </div>
        </a>
      </div>
      <!-- List Lots END -->
      ...
      <!-- <button @click="open = true" class="bg-purple-700 p-6">holaaaaaa</button> -->
    </div>
  </div>
  
<!-- Modal -->
<Modal :open="open" :lotNumber="lotNumber" @toggle="(value) => open=value" @loadLots="() => loadLots()"/>
</template>
