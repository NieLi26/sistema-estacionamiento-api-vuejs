<template>
    ...
    <div class="-my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 pr-10 lg:px-8 ">
        ...
        <div
            class="align-middle inline-block min-w-full text-center shadow overflow-hidden bg-white shadow-dashboard px-8 pt-3 rounded-bl-lg rounded-br-lg">
            <table class="min-w-full">
                <thead>
                    <tr>
                        ...
                        <th
                            class="px-6 py-3 border-b-2 border-gray-300 text-sm leading-4 text-blue-500 tracking-wider">
                            Fecha de entrada
                        </th>
                        <th
                            class="px-6 py-3 border-b-2 border-gray-300 text-sm leading-4 text-blue-500 tracking-wider">
                ...
                <tbody class="bg-white">
                    <tr v-for="{id, status, check_in, check_out, lot, licence, fare} in reserves" :key="id">
                        <td 
                        ...
                        <td
                            class="px-6 py-4 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">
                            <span :class="colorStatus(status)" class="relative inline-block px-3 py-1 font-semibold leading-tight rounded-full">
                                <span aria-hidden class="absolute inset-0  opacity-50 "></span>
                                <span class="relative text-xs">{{ status === 'an' ? 'Anulada' : status === 'bu' ? 'Ocupada': status === 'pp' ? 'Pago Pendiente': 'Finalizada' }}</span>
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-no-wrap  border-b border-gray-500 text-sm leading-5">
                            <button
                                @click="isOpen=true, dataReserve={lot, licence, fare}"
...
const loadReserves = async () => {
    try {
        ...
        const response = await CheckInAPI.getReserves();
        // setTimeout(() => {
